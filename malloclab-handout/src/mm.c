...
/* Read the size and allocated fields from address p在这里p是head */
#define GET_SIZE(p) (GET(p) & ~0x7)
#define GET_ALLOC(p) (GET(p) & 0x1)

/* Given block ptr bp, compute address of its header and footer在这里bp是p的下一位，指向真正的块的内容 */
...
static char *heap_listp;//堆的起始位置
static char *pre_listp;//上一次分配的块的下一个空闲块的位置
...
int mm_init(void)
{
    ...
    if ((heap_listp = mem_sbrk(4 * WSIZE + LISTSIZE*DSIZE)) == (void *)-1) //分配开头和各个头指针
        return -1;
    //先有20个指针，再有一个指向堆的指针
    seg_free_list = heap_listp;
    for(int i = 0; i < LISTSIZE; i++)
    {
        // seg_free_list(i) = NULL;
    }
    heap_listp += LISTSIZE*DSIZE;
    PUT(heap_listp, 0);                            /* Alignment padding 对齐填充*/
    PUT(heap_listp + (1 * WSIZE), PACK(DSIZE, 1)); /* Prologue header 序言块*/
    PUT(heap_listp + (2 * WSIZE), PACK(DSIZE, 1)); /* Prologue footer 序言块*/
    PUT(heap_listp + (3 * WSIZE), PACK(0, 1));     /* Epilogue header 普通块头部*/
    heap_listp += (2 * WSIZE);
